...
Single-purpose LAN file transfer tool with GUI. Sender chooses a folder, Receiver accepts or rejects, files move over UDP with chunk-level CRC32 and file-level MD5 verification.
...
8) Receiver writes chunks in order, validates CRC32 per chunk and MD5 after full file.
...
- **Chunking**:
  - Chunk size: 1014 bytes total (14-byte header + up to 1000-byte body).
  - Header layout:
    - Bytes 0-7: `long` sequence number (monotonic per file, starting at 0).
    - Bytes 8-11: `int` CRC32 of the entire chunk (header + body).
    - Bytes 12-13: `short` body size (0–1000).
  - Body: up to 1000 bytes of file data.
- **Checksum rules**:
  - CRC32 per chunk for corruption detection; receiver drops/requests resend of bad or missing chunks.
  - MD5 per file for integrity after full receipt; on failure receiver requests full-file resend.
- **Resend behavior**:
  - Receiver maintains a bitmap file per transferring file on disk; 1 bit per chunk (1 = received and CRC32-valid, 0 = missing). Bitmap persists until file verified and is reused when resuming after restarts.
...
- `FILE_CHUNK`: sender→receiver; includes task id, file id/index, and the 14-byte chunk header + body defined above.
...
- Reliability: Must survive packet loss and mild reordering; detect corruption via CRC32/MD5.
